...
extern {
    ...
    fn __agent_agent_delete(ag: agent_t, force: u8) -> c_void;
    ...
    fn __agent_claim_invade_with_name(
        parentagent: agent_t, constr: constraints_t, agent_name: *const char) -> agentclaim_t;

    #[link_name="set_agent_name"]
    fn __set_agent_name(claim: agentclaim_t, agent_name: *const char) -> c_void;

    #[link_name="get_agent_name"]
    fn __get_agent_name(claim: agentclaim_t, buffer: *char, size: usize) -> i32;
    ...
    fn __agent_claim_invade_or_constraints(
        parentagent: agent_t, constr_count: u8, constr: *constraints_t) -> agentclaim_t;

    #[link_name="agent_claim_invade_parentclaim"]
    fn __agent_claim_invade_parentclaim(
        parentclaim: agentclaim_t, constr: constraints_t) -> agentclaim_t;
    ...
    fn __agent_claim_get_initial_with_name(
        octoclaim: claim_t, agent_name: *const char) -> agentclaim_t;

    #[link_name="agent_claim_retreat"]
    fn __agent_claim_retreat(claim: agentclaim_t) -> c_void;

    #[link_name="agent_claim_reinvade"]
    fn __agent_claim_reinvade(claim: agentclaim_t) -> i32;

    #[link_name="agent_claim_reinvade_constraints"]
    fn __agent_claim_reinvade_constraints(
        claim: agentclaim_t, constr: constraints_t) -> i32;

    #[link_name="agent_claim_print"]
    fn __agent_claim_print(claim: agentclaim_t) -> c_void;
    ...
    fn __agent_claim_get_pecount_tile_type(
        claim: agentclaim_t, tileID: tile_id_t, _type: res_type_t) -> i32;

    #[link_name="agent_claim_get_proxyclaim_tile_type"]
    fn __agent_claim_get_proxyclaim_tile_type(
        claim: agentclaim_t, tileID: i32, _type: res_type_t) -> proxy_claim_t;
    ...
    fn __agent_constr_delete(constr: constraints_t) -> c_void;

    #[link_name="agent_constr_set_downey_speedup_curve"]
    fn __agent_constr_set_downey_speedup_curve(constr: constraints_t, A: i32, sigma: i32) -> c_void;

    #[link_name="agent_constr_set_notontile"]
    fn __agent_constr_set_notontile(constr: constraints_t, TileID: tile_id_t) -> c_void;

    #[link_name="agent_constr_set_tile_bitmap"]
    fn __agent_constr_set_tile_bitmap(constr: constraints_t, bitmap: uint16_t) -> c_void;

    #[link_name="agent_constr_set_stickyclaim"]
    fn __agent_constr_set_stickyclaim(constr: constraints_t, sticky_boolean: u8) -> c_void;

    #[link_name="agent_constr_set_vipg"]
    fn __agent_constr_set_vipg(constr: constraints_t, vipgEnable: u8) -> c_void;

    #[link_name="agent_constr_set_appclass"]
    fn __agent_constr_set_appclass(constr: constraints_t, AppClass: i32) -> c_void;

    #[link_name="agent_constr_set_appnumber"]
    fn __agent_constr_set_appnumber(constr: constraints_t, AppNr: i32) -> c_void;

    #[link_name="agent_constr_set_tile_shareable"]
    fn __agent_constr_set_tile_shareable(constr: constraints_t, is_tile_shareable: u8) -> c_void;

    #[link_name="agent_constr_set_malleable"]
    fn __agent_constr_set_malleable(
        constr: constraints_t, malleable_boolean: u8,
        resize_handler: resize_handler_t, resize_env: resize_env_t) -> c_void;

    #[link_name="agent_stresstest_agentclaim"]
    fn __agent_stresstest_agentclaim() -> c_void;

    #[link_name="agent_stresstest_agentoctoclaim"]
    fn __agent_stresstest_agentoctoclaim() -> c_void;

    #[link_name="agent_constr_set_reinvade_handler"]
    fn __agent_constr_set_reinvade_handler(
        constr: constraints_t, reinvade_handler: reinvade_handler_t) -> c_void;
    ...
    fn __agent_proxy_get_proxyagentoctoclaim(
        objects_tile: i32, octo_ucid: uint32_t) -> agentclaim_t;

    #[link_name="agent_proxy_delete_proxyagentoctoclaim"]
    fn __agent_proxy_delete_proxyagentoctoclaim(proxy_agentoctoclaim: agentclaim_t) -> c_void;
    ...
    fn __agent_claim_get_ucid(claim: agentclaim_t) -> uint32_t;
...
/// Functions to create and i32eract with Agents, Constraints and "high level" Claims.
...
///
pub fn agent_agent_delete(ag: agent_t, force: u8) -> c_void {
...
/// \return
///
pub fn set_agent_name(claim: agentclaim_t, agent_name: *const char) -> c_void {
...
///
/// * `claim` - that belongs to the agent
/// * `buffer` - where to name is copied i32o
/// * `size` - of given buffer

////// # Return Value
///
/// C-string of the name.///
pub fn get_agent_name(claim: agentclaim_t, buffer: *char, size: usize) -> i32 {
...
/// agent_constr_set_downey_speedup_curve(constraints_t constr, i32 A, i32 sigma);
///
/// # Arguments
///
...
///
/// AbstractAgentOctoClaim handle (which is castable to AgentOctoClaim* via asAOC()) if the invade operation was successful, NULL otherwise.///
pub fn agent_claim_invade_or_constraints(parentagent: agent_t, constr_count: u8, constr: *constraints_t) -> agentclaim_t {
...
/// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR to a ProxyAgentOctoClaim* via asPAOC())
///
pub fn agent_claim_retreat(claim: agentclaim_t) -> c_void {
...
/// also results in a different reinvade (i.e., constraints are not copied i32o the claim object!
...
/// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR to a ProxyAgentOctoClaim* via asPAOC())
///
pub fn agent_claim_print(claim: agentclaim_t) -> c_void {
...
/// # Note
///
/// It is not very efficient to iterate over the tiles using this function as it does not maintain any i32ernal state!
...
///
/// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR to a ProxyAgentOctoClaim* via asPAOC())
/// * `iterator` - i32eger iterating from 0 to the total number of tiles in claim.
///
...
// Functions to i32eract with Constraints
...
/// # Note
///
/// Make sure to delete all the constraint-containers you created to ac_void memory leaks!
///
/// # Arguments
///
...
///
///
pub fn agent_constr_delete(constr: constraints_t) -> c_void {
...
/// * `additionalConstraints` - Constraints handle whose Constraints should be merged i32o the existing constraints.
///
c_void agent_constr_overwrite(constraints_t constrTarget, constraints_t additionalConstraints); // TODO
...
///
c_void agent_constr_set_quantity(constraints_t constr, i32 minPEs, i32 maxPEs, res_type_t type); // sets the PE-Quantity Constraint (min, max)
...
///
/// i32uitively, A is the upper bound of parallelism the application can exploit.
...
/// as an i32eger. Thus, a value of 1.0 in the paper corresponds to a sigma value of 100 here.
///
/// # Arguments
///
...
///
pub fn agent_constr_set_downey_speedup_curve(constr: constraints_t, A: i32, sigma: i32) -> c_void {
...
///
c_void agent_constr_set_tile(constraints_t constr, tile_id_t TileID); // sets the 'this'-Place-Constraint (only cores on Tile TileID)
...
///
pub fn agent_constr_set_notontile(constr: constraints_t, TileID: tile_id_t) -> c_void {
...
///
pub fn agent_constr_set_tile_bitmap(constr: constraints_t, bitmap: uint16_t) -> c_void {
...
///
pub fn agent_constr_set_stickyclaim(constr: constraints_t, sticky_boolean: u8) -> c_void {
...
///
pub fn agent_constr_set_vipg(constr: constraints_t, vipgEnable: u8) -> c_void {
...
///
pub fn agent_constr_set_appclass(constr: constraints_t, AppClass: i32) -> c_void {
...
/// # Note
///
/// This is primarily used for the Ethernet-State-Dump i32erface.
///
/// # Arguments
///
/// * `constr` - Constraints handle for which this constraint should be set.
...
///
pub fn agent_constr_set_appnumber(constr: constraints_t, AppNr: i32) -> c_void {
...
///
pub fn agent_constr_set_tile_shareable(constr: constraints_t, is_tile_shareable: u8) -> c_void {
...
///
pub fn agent_constr_set_malleable(constr: constraints_t, malleable_boolean: u8, resize_handler: resize_handler_t, resize_env: resize_env_t) -> c_void {
...
pub fn agent_stresstest_agentclaim() -> c_void {
    __agent_stresstest_agentclaim()
}

pub fn agent_stresstest_agentoctoclaim() -> c_void {
...
///
pub fn agent_constr_set_reinvade_handler(constr: constraints_t, reinvade_handler: reinvade_handler_t) -> c_void {
...
pub fn agent_proxy_get_proxyagentoctoclaim(objects_tile: i32, octo_ucid: uint32_t) -> agentclaim_t {
...
/// # Note
///
/// Make sure to delete all the objects you created to ac_void memory leaks!
...
///
///
pub fn agent_proxy_delete_proxyagentoctoclaim(proxy_agentoctoclaim: agentclaim_t) -> c_void {
...
pub fn agent_claim_get_ucid(claim: agentclaim_t) -> uint32_t {
