...
extern {
    ...
    /// # Note
    ///
    /// When called with a ProxyAgentOctoClaim (via claim->asPAOC()) from a different tile than
    /// where the AgentOctoClaim is located, exits application with error message.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a
    ///             AgentOctoClaim* via asAOC() XOR to a ProxyAgentOctoClaim* via asPAOC())
    ...
    ///  When called with a ProxyAgentOctoClaim (via claim->asPAOC()) from a different tile than where the
    /// AgentOctoClaim is located, exits application with error message.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR to a
    ///             ProxyAgentOctoClaim* via asPAOC())
    ...
    /// if the invade operation was successful, NULL otherwise.
    ...
    /// AbstractAgentOctoClaim handle (which is castable to AgentOctoClaim* via asAOC())
    /// if the invade operation was successful, NULL otherwise.
    pub fn agent_claim_invade_with_name(parentagent: agent_t, constr: constraints_t, agent_name: *const c_char)
        -> agentclaim_t;
    ...
    /// AbstractAgentOctoClaim handle (which is castable to AgentOctoClaim* via asAOC())
    /// if the invade operation was successful, NULL otherwise.
    pub fn agent_claim_invade_or_constraints(parentagent: agent_t, constr_count: u8, constr: *mut constraints_t)
        -> agentclaim_t;
    ...
    /// # Note
    ///
    /// When called with a AgentOctoClaim (via parentclaim->asAOC()),
    /// the AgentOctoClaim handle is only valid on the tile it has been created (It is a local pointer!).
    /// When called with a ProxyAgentOctoClaim (via claim->asPAOC()) from a different tile than where
    /// the AgentOctoClaim is located, exits application with error message.
    ///
    /// # Arguments
    ///
    /// * `parentclaim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR
    ///                   to a ProxyAgentOctoClaim* via asPAOC()) of the parentclaim.
    ...
    /// See agent_claim_get_initial(claim_t octoclaim)
    ...
    /// When called with a ProxyAgentOctoClaim (via claim->asPAOC()) from a different tile
    /// than where the AgentOctoClaim is located, exits application with error message.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR
    ///             to a ProxyAgentOctoClaim* via asPAOC())
    ...
    /// # Return Value
    /// 
    /// Absolute sum of changes (i.e., gained and lost resources).
    ...
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC()
    ///             XOR to a ProxyAgentOctoClaim* via asPAOC())
    /// * `constr` - Constraints handle
    ///
    /// # Return Value 
    /// 
    ...
    /// # Note
    ///
    /// If we print the resources of a ProxyAgentOctoClaim (via claim->asPAOC()) on a distant tile, in the
    /// "tiletail" window it will be printed on the tile where the ProxyAgentOctoClaim object is located,
    /// not where the ProxyAgentOctoClaim object is located.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR
    ...
    /// # Return Value
    ///
    /// Total number of resources in given claim.
    ...
    /// # Return Value Total
    ///
    /// number of resources of specific type in given claim.
    ...
    /// # Return Value
    ///
    /// total number of resources on specific tile in given claim
    ...
    /// # Return Value
    ///
    /// Total number of resources on a specific tile of specific type in given claim.
    ...
    /// # Return Value
    ///
    /// Total number of different tiles in given claim.
    ...
    /// # Note
    ///
    /// It is not very efficient to iterate over the tiles using this function
    /// as it does not maintain any internal state!
    /// What is this function good for? Its implementation in file "AgentClaim.h" says
    /// "this function doesn't make too much sense as it is.." about it.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR
    ...
    /// # Return Value
    ///
    /// The tileID at the iterator position. Returns 0xFF if out of bounds.
    ...
    /// When set to false, only one core in the tile calling reinvade is "immune",
    /// all other tiles might lose///all* cores.
    ///
    /// # Note
    ///
    /// On by default.
    ...
    pub fn agent_constr_set_malleable(constr: constraints_t, malleable_boolean: u8, resize_handler: resize_handler_t,
                                      resize_env: resize_env_t);

    /// Assigns a reinvade handler which gets called on every reinvade of the claim which is constrained by the
    /// Constraints handle constr.
    /// If this function is not used (e.g. before an invade),
    /// the reinvade_handler will be set to NULL and therefore will not be called on reinvades (see parameter notes).
    ///
    /// # Arguments
    ///
    ...
    ///                        When reinvade_handler_t is NULL (default case),
    ///                        the handler will not be called on reinvades.
    ///			               When reinvade_handler_t is non-NULL,
    ///                        the handler will be called on every reinvade (until it is set NULL again).
    ...
    /// # Return Value
    ///
    /// The reinvade handler for the Constraints constr. Might be NULL if no reinvade handler or a NULL
    /// has been set as "reinvade handler".
    ...
    /// Returns a new ProxyAgentOctoClaim handle which allows access to its corresponding AgentOctoClaim on tile
    /// id objects_tile. The agentclaim_t is castable to AbstractAgentOctoClaim* and finally to ProxyAgentOctoClaim*
    /// via asPAOC() method.
    ///
    /// # Note
    ///
    /// The ProxyAgentOctoClaim can only be used within an iLet that needs access to ITS OWN AgentOctoClaim
    /// (the one whose ProxyClaim's were used to spawn this iLet).
    /// It may result in undefined behavior if you create a ProxyAgentOctoClaim within the same iLet that has created
    /// the AgentOctoClaim object.
    /// Retreating the AgentOctoClaim before an iLet which has a ProxyAgentOctoClaim ends, may result in undefined
    /// behavior. iRTSS allows retreating Claims without stopping the corresponding ilets.
    /// Reinvasion via agent_claim_reinvade(agentclaim_t claim) leads to a new AgentOctoClaim with a different ucid.
    /// Therefore this leads to problems with the ProxyAgentOctoClaim.
    /// See os::agent::ProxyAgentOctoClaim::reinvadeSameConstr() for more information about that.
    ///
    /// # Arguments
    ///
    /// * `objects_tile` - The tile id where the AgentOctoClaim object is located.
    ///                    All corresponding objects (AgentClaim, Constraints etc.) must be located there, too.
    /// * `octo_ucid` - The unique id (ucid) which belongs to the AgentOctoClaim that we want to access.
    ///
    /// # Return Value
    ///
    /// A new ProxyAgentOctoClaim handle (to be more precise,
    /// a AbstractAgentOctoClaim handle that is castable to a ProxyAgentOctoClaim via asPAOC() method)
    /// which allows access to its corresponding AgentOctoClaim on tile id objects_tile.
    ...
    /// Has no effect on the referenced AgentOctoClaim. One may create and delete any number of ProxyAgentOctoClaims,
    /// it will have no effect on the AgentOctoClaim.
    /// Exits application with error message when called with a AbstractAgentOctoClaim handle that is castable
    /// to a AgentOctoClaim* via asAOC().
    ///
    /// # Arguments
    ///
    /// * `proxy_agentoctoclaim` - claim AbstractAgentOctoClaim handle (which is ONLY castable to a
    ///                            ProxyAgentOctoClaim* via asPAOC()) that should be deleted.
    pub fn agent_proxy_delete_proxyagentoctoclaim(proxy_agentoctoclaim: agentclaim_t);

    /// Returns the objects tile id where the AgentOctoClaim object is located that corresponds to the
    /// ProxyAgentOctoClaim handle proxy_agentoctoclaim.
    ///
    /// # Note
    ///
    /// Exits application with error message when called with a AbstractAgentOctoClaim handle that is castable to a
    /// AgentOctoClaim* via asAOC().
    ///
    /// # Arguments
    ///
    /// * `proxy_agentoctoclaim` - The AbstractAgentOctoClaim whose objects' location we look for.
    ///                            The AgentOctoClaim object and all corresponding objects
    ///                            (AgentClaim, Constraints, ProxyClaim) must be located there
    ///                            (we don't want to scatter the objects corresponding to an iLet).
    ///                            proxy_agentoctoclaim must be castable to ProxyAgentOctoClaim via asPAOC() method.
    ///
    /// # Return Value
    ///
    /// The objects tile id where the AgentOctoClaim object is located that corresponds to the
    /// ProxyAgentOctoClaim handle proxy_agentoctoclaim.
    pub fn agent_proxy_get_objectstile(proxy_agentoctoclaim: agentclaim_t) -> i32;

    /// Returns the Ucid of the AbstractAgentOctoClaim claim. The ucid is the unique id of an os::agent::AgentClaim;
    /// the AgentClaim's referenced os::agent::AgentOctoClaim just returns its AgentClaim's ucid, so both have the same
    /// ucid. Furthermore, a os::agent::ProxyAgentOctoClaim has the same ucid as its referenced AgentOctoClaim;
    /// in the current implementation of os::agent::ProxyAgentOctoClaim, its ucid does not change during object's
    /// lifetime, so it the ProxyAgentOctoClaim class has a problem with reinvades,
    /// see os::agent::ProxyAgentOctoClaim::reinvadeSameConstr() for more information about that.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR to
    ///             a ProxyAgentOctoClaim* via asPAOC())
    ///
    /// # Return Value
    ///
    ...
    /// os::agent::AgentOctoClaim::isEmpty() just calls os::agent::AgentClaim::isEmpty() on its AgentClaim.
    ///
    /// # Arguments
    ///
    /// * `claim` - AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR
