	...
	unsafe {
		for tile in 0..get_tile_count() {
			...
			if pes != 0 {
				...
				let array_size = 136892424 * pes as usize;


				unsafe {printf("MALLOC %d, OG:%d, raw:%d\n\0".as_ptr(), array_size, 136892424, mem::size_of::<simple_ilet>());}
				let mut ILet: *mut simple_ilet = malloc(array_size) as *mut simple_ilet;

				for i in 0..pes {
					simple_ilet_init(ILet.offset(i as isize), ILetFunc, &mut sync as *mut _ as *mut c_void);
				}

				printf("Infecting %d Ilets on Tile %d\n\0".as_ptr(), pes, tile);
				proxy_infect(pClaim, ILet, pes as u32);
				free(ILet as *mut _ as *mut c_void);
			}
		}
	}
